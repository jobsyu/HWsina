...
- (void)viewDidLoad
{
    ...
    if(![[API shareInstance] isAuthorized]){
...
-(IBAction)btnActionTapped:(id)sender
{
    ...
    [[[UIActionSheet alloc] initWithTitle:nil
            ...
            destructiveButtonTitle:nil
                otherButtonTitles:@"Take photo",@"Effects!",
      @"Post Photo",@"Logout",nil] showInView:self.view];
}
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    UIImage *croppedImage = [scaledImage croppedImage:CGRectMake((scaledImage.size.width
                    -photo.frame.size.width)/2, (scaledImage.size.height - photo.frame.size.height)/2
...
-(void)uploadPhoto{
  //upload the image and the title to the web service
    [[API shareInstance] commandWithParams:[NSMutableDictionary dictionaryWithObjectsAndKeys:
                            @"upload",@"command",UIImageJPEGRepresentation(photo.image, 70)
                                           ,@"file",fldTitle.text,@"title", nil]
...
-(void)logout{
  //logout the user from the server ,and also upon success destroy the local authorization
    [[API shareInstance] commandWithParams:[NSMutableDictionary dictionaryWithObjectsAndKeys:@"logout",@"command",
                                            nil]
                              onCompletion:^(NSDictionary *json) {
                                  //logged out from server
                                  [API shareInstance].user =nil;
                                  [self performSegueWithIdentifier:@"ShowLogin" sender:nil];
