...
@implementation HWBaseStatusCellFrame
...
-(void)setStatus:(HWStatus *)status
{
    ...
    self.iconViewF = CGRectMake(iconX, iconY, iconSize.width, iconSize.height);
    ...
    if (user.verified) {
        ...
        CGFloat vipY = nameY + (nameSize.height - kMBIconH) * 0.5;
        //CGFloat vipH = nameSize.height;
        //CGFloat vipW = 14;
        self.vipViewF = CGRectMake(vipX, vipY, kMBIconW, kMBIconH);
    }
    
    /** 时间 */
    CGFloat timeX =nameX;
    CGFloat timeY =CGRectGetMaxY(self.nameLabelF) + HWStatusCellBorderW;
    CGSize timeSize = [status.createdAt sizeWithFont:HWStatusCellTimeFont];
